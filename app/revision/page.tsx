        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-ds-bg-white hover:bg-ds-bg-soft text-ds-text-strong font-bold text-xs transition-all active:scale-95 shadow-sm flex items-center gap-1.5"
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl text-ds-text-soft hover:text-ds-text-strong text-xs font-bold transition-all hover:bg-ds-bg-weak"
            >
              Restore Samples
            </button>
          </div>
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              >
                ...
                {tab.count !== undefined && (
                  <span
                    className={`text-[10px] px-1.5 rounded-full font-mono ${activeTab === tab.key ? "bg-ds-static-white/20 text-ds-static-white" : "bg-ds-bg-soft text-ds-text-sub"}`}
                  >
                    {tab.count}
                  </span>
                )}
          ...
          <section className="max-w-2xl mx-auto py-4">
            {filteredAnnotations.length === 0 ? (
              <div className="p-12 text-center bg-ds-bg-white rounded-3xl border border-ds-stroke-soft">
                <div className="w-14 h-14 rounded-2xl bg-ds-bg-weak text-ds-text-soft flex items-center justify-center mx-auto mb-4">
                  <IcNote />
                </div>
                  ...
                  <div className="flex flex-col gap-5">
                    <div className="flex items-center justify-between text-xs text-ds-text-sub">
                      <span className="font-bold">
                        Card {flashcardIndex + 1} / {filteredAnnotations.length}
                      </span>
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-0.5 rounded-full bg-ds-feature-lighter text-ds-feature-dark text-[10px] uppercase font-black tracking-wider">
                          {card.topicTitle}
                        </span>
                    ...
                    <div className="h-1.5 bg-ds-bg-soft rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${((flashcardIndex + 1) / filteredAnnotations.length) * 100}%`,
                        }}
                      />
                    </div>
                    ...
                    <div
                      ...
                      className="min-h-[300px] p-8 rounded-3xl bg-ds-bg-white border-2 border-ds-stroke-soft hover:border-ds-feature-base/40 shadow-xl transition-all duration-300 flex flex-col justify-between cursor-pointer group"
                    >
                      <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-ds-text-soft">
                        ...
                        <span className="text-ds-feature-base group-hover:underline flex items-center gap-1">
                          Click to{" "}
                          {isCardFlipped ? "see question" : "reveal note"}
                          <svg
                            className="w-3 h-3"
                            viewBox="0 0 24 24"
                            fill="none"
                      ...
                      <div className="py-6 my-auto">
                        {isCardFlipped ? (
                          <div className="space-y-3">
                            {card.note ? (
                              <div className="p-5 rounded-2xl bg-ds-feature-lighter/40 border border-ds-feature-light">
                                <span className="text-[10px] font-black text-ds-feature-dark uppercase tracking-wider block mb-2">
                                  Your Note
                                </span>
                                <MarkdownRenderer
                                  content={card.note}
                                  className="text-ds-text-strong"
                                />
                              </div>
                            ) : (
                              <div className="p-4 rounded-2xl bg-ds-bg-weak border border-ds-stroke-soft">
                                <blockquote className="text-xs text-ds-text-sub italic leading-relaxed">
                          ...
                          <div>
                            <span className="inline-flex items-center px-3 py-1 rounded-full bg-ds-feature-lighter text-ds-feature-dark font-black text-xs uppercase tracking-wider border border-ds-feature-light mb-3">
                              Question
                            </span>
                            <h3 className="text-xl font-black text-ds-text-strong font-display leading-snug">
                              {card.question}
                            </h3>
                          </div>
                        ) : (
                          <blockquote className="text-xl font-bold text-ds-text-strong leading-snug font-display">
                            &ldquo;{card.selectedText}&rdquo;
                          </blockquote>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="flex-1 py-3 rounded-2xl bg-ds-bg-white hover:bg-ds-bg-weak border border-ds-stroke-soft text-ds-text-strong font-bold text-sm transition-all active:scale-95 shadow-sm flex items-center justify-center gap-2"
                      >
                        <svg
                          className="w-4 h-4"
                          viewBox="0 0 24 24"
                      ...
                      <button
                        ...
                        className="px-5 py-3 rounded-2xl bg-ds-bg-weak hover:bg-ds-bg-soft border border-ds-stroke-soft text-ds-text-strong font-bold text-sm transition-all flex items-center gap-2"
                      >
                        <IcEdit /> Edit Note
                      </button>
                      <button
                        onClick={nextFlashcard}
                        className="flex-1 py-3 rounded-2xl bg-ds-feature-base hover:bg-ds-feature-dark text-ds-static-white font-bold text-sm transition-all active:scale-95 shadow-md shadow-ds-feature-base/20 flex items-center justify-center gap-2"
                      >
                        Next{" "}
                        <svg
                          className="w-4 h-4"
          ...
          <div className="flex gap-6 items-start">
            ...
            <aside className="w-52 shrink-0 hidden lg:flex flex-col gap-1 sticky top-6">
              ...
              >
                ...
                <span
                  className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${selectedTopic === "all" ? "bg-ds-static-white/20 text-ds-static-white" : "bg-ds-bg-soft text-ds-text-soft"}`}
                >
                  {stats.total}
                </span>
              </button>
              {availableTopics.map((topic) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full shrink-0 ${isActive ? "bg-ds-static-white/20 text-ds-static-white" : "bg-ds-bg-soft text-ds-text-soft"}`}
                    >
                      {topic.count}
                    </span>
                  </button>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 mb-5 lg:hidden scrollbar-none">
                <button
                  onClick={() => setSelectedTopic("all")}
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-bold shrink-0 border transition-all ${selectedTopic === "all" ? "bg-ds-feature-base text-ds-static-white border-ds-feature-base" : "bg-ds-bg-white text-ds-text-sub border-ds-stroke-soft"}`}
                >
                  All ({stats.total})
                </button>
                {availableTopics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => setSelectedTopic(topic.id)}
                    className={`px-3.5 py-1.5 rounded-xl text-xs font-bold shrink-0 border transition-all ${selectedTopic === topic.id ? "bg-ds-feature-base text-ds-static-white border-ds-feature-base" : "bg-ds-bg-white text-ds-text-sub border-ds-stroke-soft"}`}
                  >
                    {topic.title}{" "}
                              ...
                              <div className="px-4 py-3 bg-ds-bg-weak/30 flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-0.5">
                                  <button
                                    onClick={() =>
                                      handleCopySnippet(
                                        item.selectedText || questionTitle,
                                        item.id,
                                      )
                                    }
