  ...
  return (
    <ol className="space-y-5 mb-8" data-improve-block="step-list">
      {steps.map((step, i) => (
        ...
        >
          <span className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-ds-static-white mt-0.5 bg-ds-feature-base shadow-sm">
            {i + 1}
          </span>
          <div className="flex-1">
            <p className="text-sm text-ds-text-strong leading-relaxed">
              ...
              {step.note && (
                <span className="font-normal text-ds-text-soft">
                  {" "}
                  — {step.note}
                </span>
              )}
  ...
  return (
    <div
     ...
     data-improve-block="topic-header">
      ...
      <div>
        <h3 className={`font-black text-xl ${c.titleText} mb-2 tracking-tight`}>
          {title}
        </h3>
    ...
    >
      <div className="bg-ds-bg-white p-8 lg:p-12 rounded-3xl border border-ds-stroke-soft shadow-sm mb-12">
        ...
        <div className="flex items-center gap-4 mb-10 pb-6 border-b border-ds-stroke-soft">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl text-ds-static-white font-black text-lg bg-ds-feature-base shadow-sm shadow-ds-feature-base/15">
            {number}
          </div>
          <div>
      ...
      {solution && (
        <>
          <button
            ...
            className="text-xs font-bold text-ds-feature-base hover:text-ds-feature-dark flex items-center gap-1 transition-colors cursor-pointer"
          >
    ...
    <div className="mb-6 p-5 rounded-2xl border border-ds-stroke-soft bg-ds-bg-weak shadow-sm" data-improve-block="predict-output">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h5 className="font-bold text-sm text-ds-feature-dark">
          Predict the TypeScript Behavior
        </h5>
      </div>
      ...
      {showAnswer && (
        <div className="mt-3 p-3.5 bg-ds-bg-white rounded-xl border border-ds-stroke-soft text-sm text-ds-text-strong animate-in fade-in slide-in-from-top-2 duration-200">
          <span className="font-black text-ds-feature-base text-xs block mb-1">
            Result / Output:
          </span>
          <code className="font-mono text-xs font-semibold text-ds-text-strong whitespace-pre-wrap block mb-2">
            {answer}
          </code>
          {explanation && (
            <p className="text-xs text-ds-text-sub leading-relaxed pt-2 border-t border-ds-stroke-soft">
              {explanation}
            </p>
          )}
        </div>
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {rows.map((row, ri) => (
              <tr
                key={ri}
                className="border-b border-ds-stroke-soft last:border-0 hover:bg-ds-bg-weak/30 transition-colors"
              >
                {row.map((cell, ci) => (
