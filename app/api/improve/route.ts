...
function readSectionFileInfo(
  filePath: string,
): Pick<SectionFileInfo, "exportName" | "title" | "sectionNumber"> {
  try {
    const content = fs.readFileSync(filePath, "utf-8");
    const exportName =
      detectExportName(content) ??
      toPascalCase(path.basename(filePath, ".tsx"));
    const title =
      extractSectionContainerTitle(content) ??
      formatTitle(path.basename(filePath, ".tsx"));
...
function parseLessonPageTsx(
  ...
  const structureType: "module-grouped" | "flat-parts" = isModuleGrouped
    ? "module-grouped"
    : "flat-parts";

  // ── 2. Parse SECTIONS array ──
  const sectionsMatch = pageContent.match(
  ...
  if (isModuleGrouped) {
    ...
    const objectPattern =
      /\{\s*id:\s*["']([^"']+)["'][^}]*?stage:\s*["']([^"']+)["'][^}]*?label:\s*["']([^"']+)["'][^}]*?description:\s*["']([^"']+)["'][^}]*?(optional:\s*true[^}]*)?\}/gs;
  ...
  while ((im = importPattern.exec(pageContent)) !== null) {
    const exports = im[1].split(",").map((e) =>
      e
        .trim()
        .split(/\s+as\s+/)[0]
        .trim(),
  ...
  const renderContentMatch = pageContent.match(
    /const\s+renderContent\s*=\s*\(\)\s*=>\s*\{[\s\S]*?\n\s*\}/m,
  );
  ...
  if (isModuleGrouped) {
    ...
    const casePattern =
      /case\s+["']([^"']+)["']\s*:\s*(?:return\s*)?\(?\s*<>?([\s\S]*?)<\/?>?\s*\)?\s*;/g;
    ...
    const partCasePattern =
      /case\s+["'](part\d+)["']\s*:\s*(?:return\s*)?<([A-Z][a-zA-Z]+Section)\s*\/>/g;
      ...
      if (!fileName) {
        ...
        fileName =
          componentName
            .replace(
              /([A-Z])/g,
              (_, ch, offset) => (offset > 0 ? "-" : "") + ch.toLowerCase(),
            )
            .toLowerCase() + ".tsx";
      }
...
function scanSectionFiles(topicId: string): SectionFileMeta[] {
  ...
  for (const lessonSlug of lessonDirs) {
    ...
    for (const fileName of files) {
      ...
      const content = fs.readFileSync(filePath, "utf-8");
      const exportName =
        detectExportName(content) ?? toPascalCase(fileName.replace(".tsx", ""));
      const title =
        extractSectionContainerTitle(content) ??
        fileName.replace("-section.tsx", "");
      const sectionNumber = extractSectionContainerNumber(content);
...
export async function GET(req: NextRequest) {
  ...
  if (action === "structure") {
    ...
    if (!fs.existsSync(pageFile)) {
      return NextResponse.json(
        { error: `page.tsx not found for ${topic}/${lesson}` },
        { status: 404 },
      );
    }

    const pageContent = fs.readFileSync(pageFile, "utf-8");
    const structure = parseLessonPageTsx(
      pageContent,
      componentsDir,
      topic,
      lesson,
  ...
  if (action === "list-lessons") {
    ...
    const lessons = entries
      .filter(
        (entry) =>
          entry.isDirectory() &&
          fs.existsSync(path.join(topicDir, entry.name, "page.tsx")),
      )
      .map((entry) => ({
        id: entry.name,
  ...
  if (action === "extract-block") {
    ...
    if (!filePath || !blockType || !blockIndexParam) {
      return NextResponse.json(
        { error: "path, blockType, and blockIndex are required" },
        { status: 400 },
      );
    }
    ...
    if (pos === -1) {
      return NextResponse.json(
        { error: `Block not found (index ${blockIndex})` },
        { status: 404 },
      );
    }

    const startTagPos = pos;
    const tagNameMatch = content
      .slice(startTagPos)
      .match(/^<([a-zA-Z0-9_\.]+)/);
    if (!tagNameMatch)
      return NextResponse.json(
        { error: "Could not determine tag name" },
        { status: 500 },
      );
    ...
    if (blockType === "playground") {
      ...
      const starterMatch = blockSource.match(
        /starterCode\s*=\s*\{([A-Za-z0-9_]+)\}/,
      );
      if (starterMatch) {
        starterCodeVarName = starterMatch[1];
        // Look for the variable definition in the same file
        const varPattern = new RegExp(
          `const\\s+${starterCodeVarName}\\s*=\\s*(?:(?:\\\`([\\s\\S]*?)\\\`)|(?:\\"([^"]*)\\")|(?:\\'([^']*)\\'))`,
        );
  ...
  if (action === "extract-section") {
    ...
    if (pos === -1) {
      return NextResponse.json(
        { error: `SectionContainer not found in ${filePath}` },
        { status: 404 },
      );
    }
    ...
    if (endTagPos === -1) {
      return NextResponse.json(
        { error: "Could not find closing tag for SectionContainer" },
        { status: 500 },
      );
    }
  ...
  if (action === "file") {
    ...
    if (!isPathSafe(filePath)) {
      return NextResponse.json(
        { error: "Access denied: path is outside app/learn/" },
        { status: 403 },
      );
    }
    const abs = path.resolve(PROJECT_ROOT, filePath);
  ...
  if (action === "history") {
    ...
    if (section)
      records = records.filter((r) => r.section.fileName === section);
...
export async function POST(req: NextRequest) {
  ...
  if (action === "detect") {
    ...
    if (result.lesson && !result.section) {
      const lessonSections = allSections.filter(
        (s) => s.lessonSlug === result.lesson!.slug,
      );
    ...
    if (result.topic && !result.section) {
      const topicSections = allSections.filter(
        (s) => s.topicId === result.topic!.id,
      );
      ...
      if (similar) {
        ...
        result.signals.push(
          `Cross-lesson similarity: "${similar.fileName}" in ${similar.lessonSlug}`,
        );
        if (!result.lesson) {
          result.lesson = {
    ...
    if (result.topic && result.lesson) {
      try {
        ...
        if (fs.existsSync(pageFile)) {
          const pageContent = fs.readFileSync(pageFile, "utf-8");
          lessonStructure = parseLessonPageTsx(
            pageContent,
            componentsDir,
            result.topic.id,
            result.lesson.slug,
  ...
  if (action === "apply") {
    return NextResponse.json(
      {
        error:
          "Legacy 'apply' method is disabled to protect file integrity. Please HARD REFRESH your browser page (Ctrl+F5) to load the latest Improvement Manager.",
      },
      { status: 400 },
    );
  }
  ...
  if (action === "undo") {
    ...
    if (!isPathSafe(record.filePath))
      return NextResponse.json({ error: "Access denied" }, { status: 403 });

    fs.writeFileSync(
      path.resolve(PROJECT_ROOT, record.filePath),
  ...
  if (action === "redo") {
    ...
    if (!record.undone)
      return NextResponse.json({ error: "Not undone" }, { status: 400 });
    if (!isPathSafe(record.filePath))
      return NextResponse.json({ error: "Access denied" }, { status: 403 });
